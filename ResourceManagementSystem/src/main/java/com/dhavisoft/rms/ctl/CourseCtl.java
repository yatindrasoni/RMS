 ...
 * 
 * import java.text.Normalizer.Form; import java.util.Locale;
 * 
 ...
 * 
 * import com.dhavisoft.rms.dto.CourseDTO; import
 * com.dhavisoft.rms.exception.DuplicateRecordException; import
 * com.dhavisoft.rms.form.CourseForm; import
 * com.dhavisoft.rms.service.CourseServiceInt; import
 * com.dhavisoft.rms.validator.CourseValidator;
 * 
 *//**
 ...
 * @Controller
 * 
 * @RequestMapping(value = "/ctl/Course") public class CourseCtl extends BaseCtl
 * {
 * 
 * private static Logger log = Logger.getLogger(CourseCtl.class);
 * 
 * @Autowired private CourseServiceInt service;
 * 
 ...
 * 
 * if (id != null && id > 0) { form.populate(service.findByPK(id)); } return
 * "Course"; }
 * 
 *//**
	 * Submits Course data.
 ...
 * locale, @RequestParam(required = false) String operation,
 * 
 * @ModelAttribute("form") @Valid CourseForm form, BindingResult bindingResult,
 * Model model) {
 * 
 ...
 * 
 * try { if (OP_SAVE.equalsIgnoreCase(operation)) {
 * 
 ...
 * service.add(dto); } String msg = messageSource.getMessage("message.success",
 ...
 * model.addAttribute("success", msg); return "redirect:Course/search";
 * 
 * } else if (OP_CANCEL.equalsIgnoreCase(operation)) { CourseDTO dto =
 * (CourseDTO) form.getDto();
 * 
 * if (dto.getId() > 0) { return "redirect:/Course/search"; } else { return
 * "redirect:Course"; } } } catch (DuplicateRecordException e) {
 * 
 * String msg = messageSource.getMessage("message.courseExist", null, locale);
 * model.addAttribute("error", msg); } return "Course"; }
 * 
 *//**
 ...
 * searchList(@ModelAttribute("form") CourseForm form, Model model) {
 * 
 * int i = service.search(new CourseDTO()).size(); int size = 0; if (i % 5 == 0)
 * {
 * 
 * size = i / 5; } else { size = (i / 5) + 1; } model.addAttribute("size",
 * size);
 * 
 * model.addAttribute("list",service.search(new CourseDTO(), form.getPageNo(),
 * form.getPageSize())); return "CourseList"; }
 * 
 *//**
	 ...
	 *//*
		 ...
		 * String searchList(Locale locale, @ModelAttribute("form") CourseForm form,
		 * 
		 ...
		 * "Next".equalsIgnoreCase(operation) || "Previous".equalsIgnoreCase(operation))
		 * {
		 * 
		 * if (OP_SEARCH.equalsIgnoreCase(operation)) { pageNo = 1; } else if
		 * (OP_NEXT.equalsIgnoreCase(operation)) { pageNo++; } else if
		 * (OP_PREVIOUS.equalsIgnoreCase(operation) && pageNo > 1) { pageNo--; }
		 * 
		 * }
		 * 
		 * if (OP_NEW.equals(operation)) { return "redirect:/ctl/Course"; }
		 * 
		 ...
		 * 
		 * String msg = messageSource.getMessage("message.success", null, locale);
		 * model.addAttribute("success", msg); CourseDTO dto = (CourseDTO)
		 * form.getDto(); // NO RECORD AND DELETE RECORD if (service.search(dto, pageNo,
		 * form.getPageSize()).size() == 0) {
		 * 
		 * form.setPageNo(pageNo - 1); return "CourseList";
		 * 
		 * }
		 * 
		 ...
		 * }
		 * 
		 * // Get search attributes CourseDTO dto = (CourseDTO) form.getDto();
		 * 
